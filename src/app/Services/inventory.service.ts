...
import { Inventory } from '../Models/inventroy.model'; // Ensure the correct path and model name
...
export class InventoryService {
  ...
  getInventories(): Observable<Inventory[]> {
    return this.http.get<Inventory[]>(this.GetAllInventoriesApiUrl);
  }

  deleteInventory(inventoryId: number): Observable<void> {
    let params = new HttpParams().set("inventoryId", inventoryId.toString());
    return this.http.delete<void>(`${this.DeleteInventoryApiUrl}/${inventoryId}`, { params });
  }

  editInventory(inventory: Inventory): Observable<Inventory> {
    return this.http.put<Inventory>(`${this.EditInventoryApiUrl}/${inventory.inventoryID}`, inventory);
  }

  addInventory(inventory: Inventory): Observable<Inventory> {
    return this.http.post<Inventory>(this.AddInventoryApiUrl, inventory);
  }
}*/
import { Injectable } from '@angular/core';
