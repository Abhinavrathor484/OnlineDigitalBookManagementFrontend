...
export class AdminInventoryManageComponent implements OnInit {
  ...
  showModal: boolean = false;
  formHeader: string = "Add Inventory";
  showNotifications: boolean = false; // Toggle for notifications
  lowInventoryThreshold: number = 3000; // Example threshold
  ...
  checkInventoryLevel(): Inventory[] {
    return this.Inventories.filter(inventory => inventory.quantity < this.lowInventoryThreshold);
  }
}
